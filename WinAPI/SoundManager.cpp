...
void SoundManager::addSound(string keyName, const char* soundName, bool background, bool loop)
{
    if (loop)
    {
        if (background)
    ...
    {
        _system->createSound(soundName, FMOD_DEFAULT, 0, &_sound[_mTotalSounds.size()]);
    }
